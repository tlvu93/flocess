  ...
  function onDrag(this: any, event: DragEvent, d: unknown) {
    ...
    */
      ...
      .forEach((node) => {
        ...
        if (x_dist < SNAP_PX) {
          ...
          if (y_dist < SNAP_PX) {
            ...
            data.x = node.coordinates!.x + PUZZLE_WIDTH - X_OFFSET;
            data.y = node.coordinates!.y;

            data.coordinates!.x = node.coordinates!.x + PUZZLE_WIDTH - X_OFFSET;
            data.coordinates!.y = node.coordinates!.y;
            return;
          }
        }
      });
    ...
    if (!shouldSnap) {
      ...
      data.coordinates.x = event.x;
      data.coordinates.y = event.y;
    }
    d3.select(this)
      ...
      .attr(
        'transform',
        `scale(${SCALE_VALUE}) 
      translate(${data.coordinates.x},${data.coordinates.y})`
      );
  }
